...
const Home = () => {
  ...
  const fetchMovies = async (query = '', pageNum = 1) => {
    ...
    try {
      const url = query
        ...
        : `${API_URL}/movie/popular?api_key=${API_KEY}&language=en-US&page=${pageNum}`;
        // `${API_URL}/trending/movie/week?api_key=${API_KEY}&page=${pageNum}`;
  ...
  useEffect(() => {
    fetchMovies('', page); // Fetch trending movies on load
  }, [page]);
      ...
      <div className='title' >
        ...
        <div className='route'>
          <Link to="/" className='homee '>
           ...
           <Link to="/Top-Rated" className='Top-Rated'>
            Top-Rated
          </Link>
          <Link to="/Up-Coming" className='upcomingg'>
            Up-Coming
          </Link>
